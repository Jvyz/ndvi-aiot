# integrate_alignment_kmeans_segmentation_UNIFIED.py
"""
...
# --- K-Means + Watershed Segmentation (for 'healthy' stage) ---
def segment_leaves_kmeans_watershed(
    ...
    """
    Segments leaves using K-Means on 4-channel data, then separates with Watershed.
    Returns: Tuple ([(leaf_id, mask)], kmeans_debug_img, watershed_debug_img)
    """
    start_time = time.time()
    h_rgb, w_rgb = rgb_aligned_bgr.shape[:2]
    h_nir, w_nir = nir_aligned.shape[:2] if len(nir_aligned.shape) > 1 else (0,0)
    if h_rgb != h_nir or w_rgb != w_nir:
        print(f"  [Segment K-Means] Warning: Dimension mismatch detected. Resizing NIR to match RGB.")
        nir_aligned = cv.resize(nir_aligned, (w_rgb, h_rgb), interpolation=cv.INTER_NEAREST)
    if nir_aligned.dtype != np.uint8:
        print(f"  [Segment K-Means] Warning: NIR dtype is {nir_aligned.dtype}. Converting to uint8 for merge.")
        if np.issubdtype(nir_aligned.dtype, np.floating): nir_aligned = (np.clip(nir_aligned, 0.0, 1.0) * 255).astype(np.uint8)
        else: nir_aligned = cv.normalize(nir_aligned, None, 0, 255, cv.NORM_MINMAX, dtype=cv.CV_8U)
    h, w = rgb_aligned_bgr.shape[:2]
    if len(nir_aligned.shape) == 3: nir_aligned = nir_aligned[:,:,0]
    if rgb_aligned_bgr.dtype != nir_aligned.dtype:
         print(f"  [Segment K-Means] Error: Dtype mismatch. RGB: {rgb_aligned_bgr.dtype}, NIR: {nir_aligned.dtype}")
         return [], np.zeros_like(rgb_aligned_bgr), np.zeros_like(rgb_aligned_bgr)
    try: four_channel_img = cv.merge((rgb_aligned_bgr, nir_aligned))
    except cv.error as e:
         print(f"  [Segment K-Means] Error during cv.merge: {e}")
         return [], np.zeros_like(rgb_aligned_bgr), np.zeros_like(rgb_aligned_bgr)
    pixel_features = four_channel_img.reshape(-1, 4)
    print(f"  [Segment K-Means] Fitting K-Means with {n_clusters} clusters...")
    kmeans_start = time.time()
    kmeans = MiniBatchKMeans(n_clusters=n_clusters, random_state=42, n_init=5, batch_size=8192, max_iter=100).fit(pixel_features)
    labels = kmeans.labels_
    print(f"  [Segment K-Means] K-Means fitting done in {time.time() - kmeans_start:.3f}s")
    segmented_image = labels.reshape(h, w)
    cluster_centers = kmeans.cluster_centers_
    if np.isnan(cluster_centers).any(): print("[Error] K-Means NaN centers."); return [], np.zeros_like(rgb_aligned_bgr), np.zeros_like(rgb_aligned_bgr)
    leaf_signal_strength = cluster_centers[:, 1].astype(float) + cluster_centers[:, 3].astype(float)
    if np.all(np.isnan(leaf_signal_strength)): print("[Warning] All K-Means centers NaN G+NIR."); leaf_cluster_idx = 0
    else: leaf_cluster_idx = np.nanargmax(leaf_signal_strength)
    print(f"  [Segment K-Means] Identified leaf cluster index: {leaf_cluster_idx}")
    print(f"     Cluster Centers (BGRN):\n{np.round(cluster_centers, 2)}")
    binary_leaf_mask = np.where(segmented_image == leaf_cluster_idx, 1, 0).astype(np.uint8)
    kernel_open = np.ones((3,3), np.uint8); kernel_close = np.ones((7,7), np.uint8)
    ...
    kmeans_debug_img = np.zeros_like(rgb_aligned_bgr)
    colors = [(31, 119, 180), (255, 127, 14), (44, 160, 44), (214, 39, 40), (148, 103, 189)]; leaf_color = (34, 139, 34)
    if leaf_cluster_idx < len(colors): colors[leaf_cluster_idx] = leaf_color
    else:
        while len(colors) <= leaf_cluster_idx: colors.append(tuple(np.random.randint(50, 200, size=3)))
        colors[leaf_cluster_idx] = leaf_color
    for i in range(n_clusters): kmeans_debug_img[segmented_image == i] = colors[i % len(colors)]
    contours_cleaned, _ = cv.findContours(cleaned_mask, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    cv.drawContours(kmeans_debug_img, contours_cleaned, -1, (255, 255, 255), 1)
    ...
    if coords.size == 0:
        ...
        if np.sum(cleaned_mask) < min_leaf_area: print("  [Segment Watershed] Cleaned mask area too small."); return [], kmeans_debug_img, np.zeros_like(rgb_aligned_bgr)
        markers, _ = ndi.label(cleaned_mask); watershed_labels = markers
    else:
        mask_markers = np.zeros(distance.shape, dtype=bool); mask_markers[tuple(coords.T)] = True
    ...
    unique_labels = np.unique(watershed_labels); all_leaf_masks_watershed = []; valid_leaf_count = 0
    watershed_debug_img = np.zeros_like(rgb_aligned_bgr); np.random.seed(42)
    num_labels = len(unique_labels[unique_labels > 0])
    label_colors = {label: tuple((np.array(plt.cm.tab20(i / max(1,num_labels)))[:3] * 255).astype(int)) for i, label in enumerate(unique_labels) if label != 0}; label_colors[0] = (0, 0, 0)
    for label in unique_labels:
         if label == 0: continue
         mask = np.where(watershed_labels == label, 1, 0).astype(np.uint8); area = np.sum(mask)
         if area > min_leaf_area:
             leaf_id = f"leaf_{valid_leaf_count:03d}"
             all_leaf_masks_watershed.append((leaf_id, mask)); color = label_colors.get(label, (128, 128, 128))
             watershed_debug_img[mask == 1] = color; valid_leaf_count += 1
         else: watershed_debug_img[mask == 1] = (100, 100, 100)
    total_segment_time = time.time() - start_time
    print(f"  [Segment] K-Means + Watershed finished in {total_segment_time:.3f}s. Found {len(all_leaf_masks_watershed)} valid leaves.")
    return all_leaf_masks_watershed, kmeans_debug_img, watershed_debug_img
# --- End K-Means + Watershed ---
...
def process_and_extract_features(
    ...
    try:
        # 1. Calibration & VI Calculation per Leaf Structure
        start_time = time.time()
        ...
        # Iterate through segmented leaves
        for leaf_id, leaf_mask in all_leaf_masks:
             ...
             # Function to calculate VIs safely
...
# --- Main Execution (FIXED: UNIFIED SEGMENTATION) ---
def main():
    ...
    print(f"Dataset Root: {args.dataset_root}"); print(f"Output Dir: {args.output_dir}")
    print(f"Output CSV: {args.output_csv}"); print(f"Calibration Maps: {args.calib_maps}")
    print(f"Calibration Coords: {args.calib_coords if args.calib_coords else 'None (using normalization)'}")
    print(f"Save Debug Images: {not args.no_debug_images}")
    ...
    for pair_info in all_image_pairs_with_labels:
        ...
        try:
            ...
            # --- *** FIX: UNIFIED SEGMENTATION LOGIC *** ---
            # Remove the if/elif block that caused the data leak.
            # Use K-Means for ALL images.
            print(f"  [Segment] Using UNIFIED K-Means + Watershed logic (Stage: {pair_info['stage']})...")
            kmeans_masks, kmeans_debug, watershed_debug = segment_leaves_kmeans_watershed(
                rgb_aligned_bgr, nir_aligned
            )
            segmentation_results = (kmeans_masks, [kmeans_debug, watershed_debug])
            debug_images_list.append(("kmeans_clusters", kmeans_debug))
