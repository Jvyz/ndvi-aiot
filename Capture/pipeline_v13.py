...
*** MODIFIED: Asynchronous Processing (Firewall Fix) ***
    - A background "worker" thread now handles all slow processing.
    - /capture and /inference endpoints are now INSTANT.
    - They capture images, add a job to a queue, and return "OK".
    - This fixes the firewall timeout issue on all devices.
***
"""
...
import json
import uuid
from queue import Queue # --- NEW: For async processing queue
...
class SystemConfig:
    ...
    # --- *** CHANGE 1: Added AI Confidence Threshold *** ---
    AI_CONFIDENCE_THRESHOLD = 0.80 # (80%) - Only diseased leaves at or above this confidence trigger a "Diseased" verdict.
...
class SystemState:
    def __init__(self):
        ...
        # --- MODIFIED ---
        # Last CAPTURED raw frames (for re-analysis)
        self.rgb_frame = None
        self.nir_frame = None
        # Last PROCESSED aligned frames (for display)
...
# --- NEW: Asynchronous Processing Queue & Event ---
# This queue will hold (rgb_frame, nir_frame) tuples for the worker
processing_queue = Queue(maxsize=1)
# This event signals the worker thread to wake up and process a job
...
class ProcessingEngine:
    ...
    def process_image_pair(self, rgb_frame, nir_frame):
        ...
        --- THIS IS THE SLOW (10-15s) FUNCTION ---
        """
        try:
            with state.lock:
            ...
            if self.rf_model and self.imputer:
                try:
                    ...
                    if not all_leaf_masks:
                        ...
                        # --- *** CHANGE 2: Updated Python Verdict Logic (Confidence-Based) *** ---
                        # Determine overall plant health
                        if all_predictions:
                            # Find all leaves predicted as "Diseased" (1) that ALSO meet the high-confidence threshold.
                            ...
                            if high_confidence_diseased_leaves:
                                # If we found at least one high-confidence diseased leaf, the plant is Diseased.
                                ai_prediction = 'Diseased'
                                # The overall confidence is the average of *only* the high-confidence diseased leaves.
                                confidence = float(np.mean(high_confidence_diseased_leaves))
                            else:
                                # If there are NO high-confidence diseased leaves (even if there are low-confidence ones),
                                # the plant is considered Healthy.
                                ai_prediction = 'Healthy'
                                # Get the average confidence of all "Healthy" (0) predictions.
                                healthy_confidences = [
                                    conf for pred, conf in zip(all_predictions, all_confidences) 
                                    if pred == 0
                                ]
                                if healthy_confidences:
                                    ...
                                    # Fallback if there are only low-confidence diseased leaves (no healthy ones)
                                    confidence = float(np.mean(all_confidences)) # Average of all low-conf predictions
    ...
    <script>
        ...
        async function updateStatus() {
            try {
                ...
                // --- NEW: Refresh streams if processing is complete ---
                if (data.processing_status === 'Complete') {
                    // Check if streams have been refreshed for this capture time
                    const lastCaptureTime = data.last_capture_time;
                    if (document.body.dataset.lastRefresh !== lastCaptureTime) {
                        console.log("Processing complete, refreshing streams.");
                        refreshStreams();
                        document.body.dataset.lastRefresh = lastCaptureTime;
                    }
                }
                ...
                if (data.ai_prediction) {
                    ...
                    // --- *** This is the 2-Label JavaScript Logic *** ---
                ...
                if (data.vegetation_indices && data.vegetation_indices.ndvi_mean) {
        ...
        async function captureImage() {
            ...
            try {
                // --- THIS REQUEST IS NOW INSTANT ---
                const response = await fetch('/capture', { method: 'POST' });
                const data = await response.json();
                
                if (data.success) {
                    showMessage(data.message || 'Capture job queued!', 'success');
                    // We DON'T refresh streams here. We let updateStatus() do it.
                } else {
        ...
        async function runInference() {
            ...
            try {
                ...
                if (data.success) {
                    showMessage(data.message || 'Re-analysis job queued!', 'success');
                    // We DON'T refresh streams here. We let updateStatus() do it.
                } else {
...
def create_web_app():
    ...
    # --- *** MODIFIED: /capture is now ASYNCHRONOUS and INSTANT *** ---
    @app.route('/capture', methods=['POST'])
    def capture_image():
        try:
            # 1. Check if worker is already busy
            with state.lock:
                if "Processing" in state.processing_status or "Queued" in state.processing_status:
                    logger.warning("Processing in progress. Capture request ignored.")
                    return jsonify({'success': False, 'error': 'Processing already in progress. Please wait.'})
                state.processing_status = "Queued" # Set status
            
            # 2. Capture new frames (This is fast)
            rgb_frame, nir_frame = camera_manager.capture_frame_pair()
            
            if rgb_frame is None or nir_frame is None:
                with state.lock:
                    state.processing_status = "Error"
                    state.error_message = "Frame capture failed"
                return jsonify({'success': False, 'error': 'Frame capture failed'})
            
            # 3. Store as "last captured" for re-inference
            with state.lock:
                state.rgb_frame = rgb_frame.copy()
                state.nir_frame = nir_frame.copy()
            
            # 4. Clear queue and add new job
            while not processing_queue.empty():
                try: processing_queue.get_nowait()
                except: pass
            processing_queue.put((rgb_frame, nir_frame))
            
            # 5. Wake up the worker thread
            processing_event.set()
            
            # 6. Return IMMEDIATE success
            return jsonify({'success': True, 'message': 'Capture complete. Processing queued.'})
    ...
    # --- *** MODIFIED: /inference is now ASYNCHRONOUS and INSTANT *** ---
    ...
    def run_inference():
        try:
            with state.lock:
                # 1. Check if worker is already busy
                if "Processing" in state.processing_status or "Queued" in state.processing_status:
                    logger.warning("Processing in progress. Re-analyze request ignored.")
                    return jsonify({'success': False, 'error': 'Processing already in progress. Please wait.'})

                # 2. Check if we have frames to re-analyze
                ...
                # 3. Get the last captured frames
                rgb_frame = state.rgb_frame.copy()
                nir_frame = state.nir_frame.copy()
                state.processing_status = "Queued for Re-analysis"
            ...
            # 6. Return IMMEDIATE success
            return jsonify({'success': True, 'message': 'Re-analysis job queued.'})
...
# ====================== NEW: PROCESSING WORKER THREAD ======================
def processing_worker(queue, event):
    """
    A background worker thread that waits for a signal,
    processes one image pair from the queue, and goes back to sleep.
    """
    logger.info("Processing worker thread started and waiting for jobs.")
    while True:
        try:
            # 1. Sleep until event is set
            event.wait() 
            
            # 2. Event received, get job from queue
            try:
                rgb_frame, nir_frame = queue.get(timeout=1.0)
            except:
                # Queue was empty, false alarm.
                event.clear() # Clear the signal
                continue 

            # 3. We have a job. Clear the event signal
            event.clear()
            
            try:
                logger.info("Worker: New job received. Starting processing...")
                # 4. Run the SLOW processing
                # processing_engine is a global object
                ...
                if success:
                    ...
                    # state.processing_status will be "Error"
            
            except Exception as e:
                logger.error(f"Worker: Unhandled exception in processing: {e}")
                with state.lock:
                    state.processing_status = "Error"
                    state.error_message = str(e)
            
            # 5. Job is done, loop back and .wait() (sleep)
...
def main():
    ...
    # --- *** NEW: Start the background processing worker *** ---
    logger.info("Starting background processing worker...")
    worker_thread = threading.Thread(
        target=processing_worker,
        args=(processing_queue, processing_event),
        daemon=True
    )
    worker_thread.start()
    ...
    try:
        app = create_web_app()
        # threaded=True is CRITICAL for this to work
