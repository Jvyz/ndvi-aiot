...
# ====================== DASHBOARD HELPER FUNCTIONS (MODIFIED) ======================
...
def create_heatmap_bgr(vi_array: np.ndarray, mask: np.ndarray, cmap_name: str = 'RdYlGn', vmin: float = -1.0, vmax: float = 1.0, add_colorbar: bool = True) -> np.ndarray:
    ...
    if add_colorbar:
        ...
        cb_padding = 10
        total_width = colored_bgr_uint8.shape[1] + cb_width + cb_padding
        ...
        for i in range(cb_height):
            final_image[i, colored_bgr_uint8.shape[1] + cb_padding : colored_bgr_uint8.shape[1] + cb_padding + cb_width] = gradient_colors_bgr[i]
        ...
        cv.putText(final_image, f"{vmax:.1f}", 
                   (colored_bgr_uint8.shape[1] + cb_padding + cb_width + 5, 20),
        ...
        if vmin < 0.0 < vmax:
            ...
            cv.putText(final_image, "0.0", 
                       (colored_bgr_uint8.shape[1] + cb_padding + cb_width + 5, mid_y),
                       font, font_scale, text_color, font_thickness, cv.LINE_AA)
        # Bottom (vmin) label
        cv.putText(final_image, f"{vmin:.1f}", 
                   (colored_bgr_uint8.shape[1] + cb_padding + cb_width + 5, cb_height - 10),
                   font, font_scale, text_color, font_thickness, cv.LINE_AA)
...
def add_title_to_image(image: np.ndarray, title_text: str, title_height: int = 40, font_scale: float = 0.9) -> np.ndarray:
    """Adds a black title bar with white text to the top of an image. (Still present but not used in new UI HTML)"""
    pass
