...
# Local modules
# Assuming these helper files exist
...
class SystemConfig:
    ...
    # WISE-IoT DataHub
    DATAHUB_HOST = "wise-iot.advantech.com"
    DATAHUB_PORT = 1883
    DEVICE_ID = "rpi5_plant_monitor_001"
    API_KEY = "your_api_key_here"
    
    # GPS
    GPS_UPDATE_INTERVAL = 5.0  # seconds (Now that L76X.py has a timeout, this thread will reliably run every 5s)
...
class ProcessingEngine:
    ...
    def process_image_pair(self, rgb_frame, nir_frame):
        ...
        try:
            ...
            # 2. NDVI calculation for visualization
            ndvi_array = compute_ndvi_from_rgb_ir(aligned_rgb, aligned_nir, normalize=True)
            ndvi_colored = colorize_ndvi(ndvi_array)
            mean_ndvi = float(np.mean(ndvi_array))
            
            with state.lock:
                state.processing_status = "Segmenting leaves..."
            
            # 3. Advanced leaf segmentation using K-means + Watershed
            ...
            # 4. Advanced AI Inference with structural analysis
            ai_prediction = None
            confidence = 0.0
            ...
            if self.rf_model and self.imputer and all_leaf_masks:
                try:
                    ...
                    # Prepare calibrated reflectance data
                    ...
                    segmentation_result = aligned_rgb.copy()
                    ...
                    for i, (leaf_id, leaf_mask) in enumerate(all_leaf_masks):
                        ...
                        if interveinal_mask is not None:
                            inter_mask_bool = interveveinal_mask.astype(bool)
                        ...
                        cv.drawContours(segmentation_result, contours, -1, color, 3)
                        ...
                        if contours:
                            ...
                            if M["m00"] != 0:
                                ...
                                cv.putText(segmentation_result, label_text, (cX-50, cY), 
                    ...
                    segmentation_result = watershed_debug if watershed_debug is not None else aligned_rgb
            
            # 5. Calculate overall vegetation indices for display
            red_channel = aligned_rgb[:,:,2]
            red_refl = (red_channel.astype(np.float32) / 255.0)
            nir_refl = (aligned_nir.astype(np.float32) / 255.0)
            
            vi_data = self.preprocessor.calculate_zhang_vegetation_indices(red_refl, nir_refl)
...
class GPSManager:
    ...
    def initialize(self):
        ...
        try:
            # L76X.__init__ now handles all configuration and baudrate setting
            self.gps = L76X() 
    ...
    def update_loop(self):
        ...
        while self.running:
            try:
                if self.gps:
                    # This call is now non-blocking (max 10s) and will return control
                    ...
                    with state.lock:
                        state.gps_data.update({
                            ...
                            'altitude': 0.0, # Altitude data is often in GNGGA, but setting to 0.0 for now
                ...
                # The thread sleeps for 5 seconds, ensuring the L76X_Gat_GNRMC() 
                # (which takes 0-10s) doesn't hog the CPU indefinitely.
...
# ====================== DATAHUB CONNECTOR ======================
# (Rest of DataHubConnector and Web Interface code remains unchanged)
...
def create_web_app():
            ...
            while True:
                with state.lock:
                    if stream_type == 'rgb' and state.rgb_jpg is not None:
                        ...
                        cv.putText(placeholder, f"{stream_type.upper()}", (250, 240), 
                                 cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                        _, frame_data = cv.imencode('.jpg', placeholder)
    ...
    def capture_image():
        try:
            ...
            _, rgb_jpg = cv.imencode('.jpg', rgb_frame)
            _, nir_jpg = cv.imencode('.jpg', cv.cvtColor(nir_frame, cv.COLOR_GRAY2BGR))
            ...
            if success:
                ...
                with state.lock:
                    if state.ndvi_image is not None:
                        _, result_jpg = cv.imencode('.jpg', state.ndvi_image)
    ...
    def run_inference():
        try:
            with state.lock:
                if state.aligned_rgb is None or state.aligned_nir is None:
                    return jsonify({'success': False, 'error': 'No processed images available'})
            
            # Re-run processing to ensure latest AI results
            success = processing_engine.process_image_pair(state.rgb_frame, state.nir_frame)
...
def main():
    ...
    print(f"   Listening on: http://{SystemConfig.WEB_HOST}:{SystemConfig.WEB_PORT}")
