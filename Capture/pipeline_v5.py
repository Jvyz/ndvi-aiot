...
def add_title_to_image(image: np.ndarray, title_text: str, title_height: int = 40, font_scale: float = 0.9) -> np.ndarray:
    ...
    # This helper is no longer needed if we put labels in HTML
    # but it's good to keep. We will use HTML labels instead.
    pass
...
class SystemState:
    def __init__(self):
        ...
        self.confidence = 0.0
        self.segmentation_result = None # This is the full-res AI-annotated image
        ...
        self.result_jpg = None        # AI Prediction (repurposed)
...
class ProcessingEngine:
    ...
    def process_image_pair(self, rgb_frame, nir_frame):
        ...
        try:
            ...
            # 3. AI Prediction
            _, pred_jpg_bytes = cv.imencode('.jpg', cv.resize(ai_prediction_image, target_size), jpeg_quality)
            # 4. NDVI Heatmap
            panel_ndvi_bgr = create_heatmap_bgr(ndvi_array_2d, vmin=-1.0, vmax=1.0)
            _, ndvi_jpg_bytes = cv.imencode('.jpg', cv.resize(panel_ndvi_bgr, target_size, interpolation=cv.INTER_NEAREST), jpeg_quality)
            # 5. RDVI Heatmap
            panel_rdvi_bgr = create_heatmap_bgr(rdvi_array_2d, vmin=-1.5, vmax=1.5)
            _, rdvi_jpg_bytes = cv.imencode('.jpg', cv.resize(panel_rdvi_bgr, target_size, interpolation=cv.INTER_NEAREST), jpeg_quality)
            # 6. Segmentation
            ...
            with state.lock:
                ...
                state.segmentation_result = ai_prediction_image
                
                # Store JPEGs for streaming
                state.rgb_jpg = rgb_jpg_bytes.tobytes()
                state.nir_jpg = nir_jpg_bytes.tobytes()
                state.result_jpg = pred_jpg_bytes.tobytes() # 'result' stream is AI Prediction
...
<body>
    <div class="container">
        ...
        <div class="camera-grid">
            ...
            <div class="camera-view">
                ...
                <img src="/stream/result" alt="AI Prediction Result" id="result-stream">
            </div>
        </div>
    ...
    <script>
        ...
        function refreshStreams() {
            ...
            document.getElementById('result-stream').src = '/stream/result?' + timestamp;
            ...
            while True:
                ...
                with state.lock:
                    if stream_type == 'rgb' and state.rgb_jpg is not None:
                        ...
                        frame_data = state.result_jpg # This is the AI Prediction
