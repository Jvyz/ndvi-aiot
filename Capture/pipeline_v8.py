...
# --- This now imports the *robust* segmentation function ---
...
class SystemConfig:
    ...
    USE_AUTO_EXPOSURE = False  # Start in Manual Mode
    INITIAL_EXPOSURE = 4000    # Initial manual exposure (in µs). Lower this if it's too bright.
...
def create_heatmap_bgr(vi_array: np.ndarray, mask: np.ndarray, cmap_name: str = 'RdYlGn', vmin: float = -1.0, vmax: float = 1.0, add_colorbar: bool = True) -> np.ndarray:
    ...
    if add_colorbar:
        ...
        cb_padding = 10
        total_width = colored_bgr_uint8.shape[1] + cb_width + cb_padding + 40 # Extra 40px for text
        ...
        text_x_pos = cb_x_end + 5 # Position text to the right of the bar

        # Top (vmax) label
        cv.putText(final_image, f"{vmax:.1f}", (text_x_pos, 20),
                   font, font_scale, text_color, font_thickness, cv.LINE_AA)
        # Middle (0.0) label
        if vmin < 0.0 < vmax:
            mid_y = int(cb_height * (vmax - 0.0) / (vmax - vmin)) # Calculate position of 0.0
            cv.putText(final_image, "0.0", (text_x_pos, mid_y),
                       font, font_scale, text_color, font_thickness, cv.LINE_AA)
        # Bottom (vmin) label
        cv.putText(final_image, f"{vmin:.1f}", (text_x_pos, cb_height - 10),
                   font, font_scale, text_color, font_thickness, cv.LINE_AA)
...
class SystemState:
    def __init__(self):
        ...
        # --- NEW: Exposure State ---
        self.auto_exposure = SystemConfig.USE_AUTO_EXPOSURE
        self.manual_exposure = SystemConfig.INITIAL_EXPOSURE
...
class CameraManager:
    ...
    def initialize_cameras(self):
        """Initialize RGB and NIR cameras with initial exposure settings"""
        ...
        try:
            ...
            # Set initial camera controls based on state
            with state.lock:
                auto = state.auto_exposure
                exposure = state.manual_exposure
            
            self.set_exposure_mode(auto, exposure) # Apply initial settings
    ...
    # --- NEW METHOD to change exposure on the fly ---
    def set_exposure_mode(self, auto: bool, exposure_us: int):
        """Sets the exposure mode (auto or manual) on both cameras."""
        if not CAMERA_AVAILABLE:
            return # Do nothing in simulation mode

        try:
            frame_period_us = int(1_000_000 / SystemConfig.FPS)
            
            if auto:
                logger.info("Setting cameras to AUTO exposure")
                controls = {
                    "AeEnable": True,
                    "AnalogueGain": 1.0, # Let auto-exposure handle gain
                    "FrameDurationLimits": (frame_period_us, frame_period_us),
                }
            else:
                logger.info(f"Setting cameras to MANUAL exposure: {exposure_us}µs")
                controls = {
                    "AeEnable": False,
                    "ExposureTime": int(exposure_us),
                    "AnalogueGain": 1.0, # Keep gain low for manual exposure
                    "FrameDurationLimits": (frame_period_us, frame_period_us),
                }
            ...
            with state.lock:
                state.auto_exposure = auto
                state.manual_exposure = int(exposure_us)
            
        except Exception as e:
            logger.error(f"Failed to set exposure controls: {e}")
...
class ProcessingEngine:
    ...
    def process_image_pair(self, rgb_frame, nir_frame):
        ...
        try:
            ...
            # 4. Advanced leaf segmentation (using ROBUST HSV+NIR function)
            all_leaf_masks, hsv_nir_debug, watershed_debug = segment_leaves_kmeans_watershed(
            ...
            # Create a combined mask for all detected leaves for heatmaps
            combined_leaf_mask = np.zeros(aligned_rgb.shape[:2], dtype=np.uint8)
            if all_leaf_masks:
                for leaf_id, leaf_mask in all_leaf_masks:
                    combined_leaf_mask = cv.bitwise_or(combined_leaf_mask, leaf_mask)
            else:
                # Fallback: if segmentation finds nothing, use the HSV+NIR mask
                # This ensures heatmaps still show *something* if leaves are present but not separated
                if np.any(hsv_nir_debug):
                    combined_leaf_mask = cv.cvtColor(hsv_nir_debug, cv.COLOR_BGR2GRAY)
                    _, combined_leaf_mask = cv.threshold(combined_leaf_mask, 1, 255, cv.THRESH_BINARY)
            ...
            # This is the panel for AI predictions (RGB + overlays)
                ...
                try:
                    ...
                    for i, (leaf_id, leaf_mask) in enumerate(all_leaf_masks):
                        ...
                        if contours:
                            ...
                            if M["m00"] != 0:
                                cX = int(M["m10"] / M["m00"])
                                cY = int(M["m01"] / M["m0CMYK_"] * (0.8 + 0.1 * i))
                    ...
                    ai_prediction_image = watershed_debug if watershed_debug is not None else aligned_rgb
            ...
            # --- *** Encode all 6 panels for streaming *** ---
            ...
            # 2. Aligned NIR
            _, nir_jpg_bytes = cv.imencode('.jpg', cv.resize(aligned_nir, target_size), jpeg_quality)
            ...
            # 6. Segmentation (original color-coded segmentation from watershed)
            _, seg_jpg_bytes = cv.imencode('.jpg', cv.resize(watershed_debug, target_size, interpolation=cv.INTER_NEAREST), jpeg_quality)
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .exposure-controls input[type="range"]:disabled {
            opacity: 0.5;
        }
        .exposure-controls span {
            font-size: 1.1em;
            font-weight: 700;
            color: #6f42c1;
            min-width: 60px;
...
<body>
    <div class="container">
        ...
        <div class="exposure-controls">
            <label for="auto-exposure-toggle">
                <input type="checkbox" id="auto-exposure-toggle"> Auto-Exposure
            </label>
            <label for="exposure-slider" style="margin-left: 30px;">
                Manual Exposure:
            </label>
            <input type="range" min="500" max="20000" step="100" id="exposure-slider">
            <span id="exposure-value">--</span>µs
        </div>
    ...
    <script>
        ...
        const exposureValueText = document.getElementById('exposure-value');
        const autoExposureToggle = document.getElementById('auto-exposure-toggle');

        // --- NEW: Update status now includes exposure ---
        async function updateStatus() {
            try {
                ...
                // --- NEW: Sync Exposure Controls ---
                if (data.auto_exposure !== undefined) {
                    autoExposureToggle.checked = data.auto_exposure;
                    exposureSlider.disabled = data.auto_exposure;
                    exposureValueText.textContent = data.manual_exposure;
                    if (document.activeElement !== exposureSlider) {
        ...
        // --- NEW: Send Exposure Update to Server ---
        async function setExposureMode() {
            const auto = autoExposureToggle.checked;
            const exposure = exposureSlider.value;
            
            // Update UI immediately
            exposureSlider.disabled = auto;
            exposureValueText.textContent = exposure;

            try {
                const response = await fetch('/set_exposure_mode', {
                    method: 'POST',
                ...
                const data = await response.json();
                if (data.success) {
                    showMessage(`Exposure updated: ${data.mode}`, 'success');
                    exposureValueText.textContent = data.exposure;
                } else {
                    showMessage('Failed to update exposure', 'error');
                }
            } catch (error) {
                showMessage('Exposure request failed: ' + error.message, 'error');
            }
        }

        // --- NEW: Event Listeners for Exposure ---
        autoExposureToggle.addEventListener('change', setExposureMode);
        exposureSlider.addEventListener('change', setExposureMode);
        // Also update text while sliding
    ...
    def get_status():
        with state.lock:
            return jsonify({
                ...
                # --- NEW: Return exposure state ---
                'auto_exposure': state.auto_exposure,
                'manual_exposure': state.manual_exposure
            })
    
    # --- NEW: Endpoint to set exposure ---
    @app.route('/set_exposure_mode', methods=['POST'])
    def set_exposure_mode():
        data = request.json
        try:
            auto = data.get('auto', False)
            exposure = data.get('exposure', 10000)
            
            # Call the camera manager to update settings
            camera_manager.set_exposure_mode(auto, exposure)
            
            mode_str = "Auto" if auto else f"Manual ({exposure}µs)"
            return jsonify({'success': True, 'mode': mode_str, 'exposure': exposure})
        ...
        try:
            with state.lock:
                payload = {
                    ...
                    # --- NEW: Send exposure settings with data ---
                    'auto_exposure': state.auto_exposure,
                    'manual_exposure': state.manual_exposure
                }
