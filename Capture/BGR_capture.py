...
Usage:
  python dataset_capture.py --rgb 0 --ir 1

Web interface provides buttons to capture:
  - Healthy leaf images
  - Infected leaf images
"""
...
def setup_camera(cam_index, size, fps, exposure_us, analogue_gain, awb=True, ae=True, warmup_s=0.4):
...
class SharedState:
    def __init__(self):
        ...
        self.metadata_file = None  # CSV file path
...
<div class="controls">
  <div class="instructions">
    ...
    <ul>
      <li><strong>Sample preparation:</strong> Detach individual leaves, place on black fabric background</li>
      <li><strong>Lighting:</strong> Use consistent lighting (halogen/LED), no direct sunlight</li>
      <li><strong>Healthy:</strong> Leaves from disease-free plants, no visible symptoms</li>
      <li><strong>Infected:</strong> Leaves showing bacterial spot symptoms (dark spots, yellowing)</li>
      <li><strong>Image quality:</strong> Check alignment quality (NCC) before capture</li>
    </ul>
  </div>
...
def create_app(args):
    ...
    def capture():
        ...
        category = request.args.get("category", "healthy")  # 'healthy' or 'infected'
        ...
        # Save with category in filename
        ...
        return jsonify({
            "ok": True,
            "msg": f"Saved {category} leaf pair to {category_dir}/",
...
def log_metadata(timestamp, category, rgb_path, nir_path, ncc_score, align_time_ms, sample_number):
    ...
    # Get image dimensions from RGB path
    try:
    ...
    try:
        with open(S.metadata_file, 'a', newline='') as f:
            ...
            writer.writerow([
                ...
                os.path.basename(nir_path),
                f"{ncc_score:.4f}",
                f"{align_time_ms:.2f}",
                width,
                height,
                ''  # notes field - can be filled manually later
            ])
    except Exception as e:
...
def capture_worker(args):
    ...
    rgb_cam = setup_camera(
        ...
        args.exp_us_rgb, args.gain_rgb, awb=True
    )
    ir_cam = setup_camera(
        args.ir, (args.W, args.H), args.fps,
        args.exp_us_ir, args.gain_ir, awb=True
    )
    ...
    try:
        while True:
            ...
            # Encode to JPEG
            _, j_rgb = cv.imencode(".jpg", rgb_vis, [cv.IMWRITE_JPEG_QUALITY, 85])
            _, j_ir = cv.imencode(".jpg", ir_vis, [cv.IMWRITE_JPEG_QUALITY, 85])
            _, j_overlay = cv.imencode(".jpg", overlay, [cv.IMWRITE_JPEG_QUALITY, 85])
