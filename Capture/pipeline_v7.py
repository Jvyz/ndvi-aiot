...
class ProcessingEngine:
    ...
    def process_image_pair(self, rgb_frame, nir_frame):
        ...
        try:
            ...
            if all_leaf_masks: # Only if leaves were found
                for leaf_id, leaf_mask in all_leaf_masks:
                    combined_leaf_mask = cv.bitwise_or(combined_leaf_mask, leaf_mask)
            else:
                # Fallback: if no leaves, create a mask of the whole image (or a default)
                # For now, let's keep it as an empty mask, so the background color is applied
                pass
