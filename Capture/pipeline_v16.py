...
1.  VI heatmaps (NDVI/RDVI) now use 'colorize_ndvi' for clarity (like runtime_align_and_ndvi.py).
2.  '/capture' button:
    - Runs alignment and VI calculation ONLY (no segmentation/AI).
    - Automatically sends data to DataHub.
3.  '/inference' (Re-Analyze) button:
    - This is now the ONLY way to trigger the full segmentation and AI pipeline.
4.  Web host remains accessible on the local network (host='0.0.0.0').
"""
...
# --- REMOVED 'create_heatmap_bgr' function as requested ---
# (It was lines 92-177 in the original file)
...
class ProcessingEngine:
    ...
    # --- *** MODIFIED: Added 'run_ai' flag *** ---
    def process_image_pair(self, rgb_frame, nir_frame, run_ai: bool = True):
        """
        Process RGB+NIR frame pair.
        - ALWAYS runs alignment and VI calculation.
        - ONLY runs AI/segmentation if 'run_ai' is True.
        """
        try:
            with state.lock:
                state.processing_status = "Aligning images..."
            
            # 1. Image alignment (Uses v2_preprocessing)
            ...
            # 4. ALWAYS run the FAST mask generation for segmentation panel fallback
            cleaned_mask, hsv_nir_debug = create_hsv_nir_mask(
                aligned_rgb, aligned_nir
            )
            logger.info("Fast HSV+NIR mask generated")
            ...
            # --- *** MODIFIED: AI pipeline is now conditional *** ---
            if run_ai:
                ...
                logger.info("AI analysis skipped by request.")
                ai_prediction_image = aligned_rgb.copy()
                ai_prediction = None
                confidence = 0.0
            ...
            if watershed_debug_img is None:
                # logger.info("Using simple mask for segmentation view.") # Too verbose
            ...
            # --- *** MODIFIED: Use colorize_ndvi (from ndvi_utils) for clarity *** ---
            
            # 4. NDVI Heatmap
            # colorize_ndvi (imported from ndvi_utils) handles normalization and colormap
            panel_ndvi_bgr = colorize_ndvi(ndvi_array_2d)
            _, ndvi_jpg_bytes = cv.imencode('.jpg', cv.resize(panel_ndvi_bgr, target_size, interpolation=cv.INTER_LINEAR), jpeg_quality)
            
            # 5. RDVI Heatmap
            # Use colorize_ndvi for RDVI as well. It will normalize the RDVI range.
            panel_rdvi_bgr = colorize_ndvi(rdvi_array_2d)
            _, rdvi_jpg_bytes = cv.imencode('.jpg', cv.resize(panel_rdvi_bgr, target_size, interpolation=cv.INTER_LINEAR), jpeg_quality)
    ...
    <script>
        ...
        async function updateStatus() {
            try {
                ...
                if (data.ai_prediction) {
                    ...
                    // Hide health status if no AI prediction
                ...
                if (data.ai_prediction) {
                    ...
                    `;
                     ...
                     document.getElementById('ai-results').innerHTML = `<p>Press 'Re-Analyze' to run AI.</p>`;
                }
        ...
        async function captureImage() {
            ...
            try {
                ...
                if (data.success) {
                    // --- MODIFIED: Message now comes from server ---
                    showMessage(data.message || 'Capture complete!', 'success');
        ...
        // --- *** MODIFIED: This function is more forceful to fix the update bug *** ---
        function refreshStreams() {
            const timestamp = new Date().getTime();
            const streams = ['rgb', 'nir', 'ai_prediction', 'ndvi', 'rdvi', 'segmentation'];
            
            streams.forEach(function(stream) {
                // Construct the full ID, e.g., "rgb-stream"
                const elementId = stream + '-stream';
                
                // For 'ai_prediction', the ID is 'ai_prediction-stream'
                // This is handled correctly by the line above.
                
                const imgElement = document.getElementById(elementId);
                
                if (imgElement) {
                    // 1. Force the browser to disconnect from the old stream
                    imgElement.src = ''; 
                    
                    // 2. Use a short timeout to set the new stream source
                    // This gives the browser time to process the 'src' change
                    setTimeout(function() {
        ...
        // --- *** MODIFIED: Removed sendToDataHub() function *** ---
...
def create_web_app():
    ...
    # --- *** MODIFIED: Runs VIs only (no AI) and sends to DataHub *** ---
    ...
    def capture_image():
        try:
            ...
            # Process images (run_ai=False)
            success = processing_engine.process_image_pair(rgb_frame, nir_frame, run_ai=False)
            
            if success:
                logger.info("Capture successful, now sending to DataHub...")
                try:
                    ...
                    with state.lock:
                        payload = {
                            ...
                            'vegetation_indices': state.vegetation_indices,
                            'ai_prediction': state.ai_prediction, # Will be None
                            'confidence': state.confidence,     # Will be 0.0
                    ...
                    if datahub_success:
                        return jsonify({'success': True, 'message': 'Image processed and data sent to DataHub'})
                    else:
                        return jsonify({'success': True, 'message': 'Image processed, but DataHub send failed'})
                        
                except Exception as e:
                    logger.error(f"DataHub send after capture failed: {e}")
                    return jsonify({'success': True, 'message': 'Image processed, but DataHub send failed with exception'})
    ...
    # --- *** MODIFIED: Now the only route to run AI *** ---
    ...
    def run_inference():
        try:
            ...
            # Re-run processing (run_ai=True)
            success = processing_engine.process_image_pair(rgb_frame, nir_frame, run_ai=True)
    ...
    # --- *** MODIFIED: Removed /send_datahub route *** ---
...
def main():
    ...
    try:
        app = create_web_app()
        # --- This setting allows access from other devices on the network ---
