...
*** REVISED (v12) for Responsiveness ***
1.  Added a background 'processing_worker_thread' (like BGR_capture.py).
2.  This worker continuously captures, aligns, and calculates VIs/Masks.
3.  Streams (RGB, NIR, NDVI, RDVI, Seg) are now LIVE and non-blocking.
4.  '/capture' button is now INSTANT: it sends the latest live data to DataHub.
5.  '/inference' button is the ONLY slow-blocking call, running AI on the latest data.
"""
...
class SystemState:
    def __init__(self):
        ...
        # Full-resolution data for processing
        self.live_aligned_rgb = None
        self.live_aligned_nir = None
        self.live_vi_data = None # Holds ZhangVegetationIndices object
        
        # JPEG streams (6 panels)
        self.rgb_jpg = None
        self.nir_jpg = None
        self.ai_prediction_jpg = None # Updated by AI button
        ...
        self.vegetation_indices_stats = {} # Dict of mean/std
...
class ProcessingEngine:
    ...
    def run_ai_inference(self, aligned_rgb, aligned_nir, red_refl_01, nir_refl_01):
        """
        Runs the HEAVY AI part (segmentation + model) on captured frames.
        Returns the annotated image and prediction results.
        """
        if not self.rf_model or not self.imputer:
            logger.warning("AI analysis skipped: Model not loaded.")
            return aligned_rgb, None, 0.0, 0, None # Return defaults
        ...
        try:
            ...
            if not all_leaf_masks:
                ...
                return watershed_debug_img, None, 0.0, 0, watershed_debug_img # Return debug image
            ...
            # 4. Determine overall plant health
            ai_prediction = None
            confidence = 0.0
            if all_predictions:
            ...
            return aligned_rgb, None, 0.0, 0, hsv_nir_debug # Fallback
...
def encode_stream_image(frame, stream_name=""):
    """Encodes a frame to JPEG bytes, with error handling."""
    if frame is None:
    ...
    # Resize to standard stream size
...
def processing_worker_thread(camera_manager, preprocessor):
    ...
    Mimics the 'BGR_capture.py' architecture for a responsive UI.
    """
    ...
    while True:
        try:
            ...
            # 5. Create heatmap visualizations (using the fast colorize_ndvi)
            ndvi_heatmap = colorize_ndvi(vi_data.ndvi)
            rdvi_heatmap = colorize_ndvi(vi_data.rdvi) # This will auto-normalize RDVI
            ...
            with state.lock:
                # Store full-res data for AI button
                ...
                # Encode and store JPEGs for live streams
...
<body>
    ...
    <script>
        ...
        async function captureAndSend() {
            ...
            try {
                // This route is now very fast, just sends data
        ...
        // This function now refreshes specific streams
        function refreshStreams(stream_ids) {
            ...
            stream_ids.forEach(function(stream) {
                ...
                if (imgElement) {
                    imgElement.src = ''; // Force disconnect
                    setTimeout(function() {
        ...
        // Run initial status update and set placeholder text
        window.onload = () => {
            updateStatus();
...
def create_web_app():
        ...
        def generate_stream():
            while True:
                ...
                if frame_data is None:
                    # Generate a placeholder if worker hasn't run yet
                ...
                yield (b'--frame\r\n'
                       ...
                       frame_data + b'\r\n')
                # This sleep is CRITICAL to prevent the server from locking up
                # and allow the background worker to run.
                time.sleep(0.05)
    ...
    # --- *** MODIFIED: /capture is now a fast DataHub send *** ---
    ...
    def capture_and_send():
        try:
            ...
            with state.lock:
                ...
                # Prepare data payload from the live worker data
                payload = {
                    'timestamp': datetime.now().isoformat(),
                    'device_id': SystemConfig.DEVICE_ID,
                    'ndvi_value': state.current_ndvi_value,
                    'vegetation_indices': state.vegetation_indices_stats,
                    'ai_prediction': state.ai_prediction, # Will be last-run AI prediction
                    'confidence': state.confidence,
                    'gps_data': state.gps_data.copy(),
                    'session_id': str(uuid.uuid4())
                }
                state.last_capture_time = datetime.now() # Log this as the 'capture' time
    ...
    # --- *** MODIFIED: /inference is now the only SLOW, BLOCKING call *** ---
    ...
    def run_inference():
        try:
            ...
            with state.lock:
                ...
                state.vegetation_indices_stats['leaves_detected'] = leaves_found # Add leaf count
                
                # Update the specific streams that show AI results
...
def main():
    ...
    print("=" * 60)
    print("🌱 AIoT Plant Health Monitor System (v12 - Async)")
    ...
    # --- *** NEW: Start the background processing thread *** ---
    ...
    try:
        ...
        app.run(host=SystemConfig.WEB_HOST, port=SystemConfig.WEB_PORT, 
                debug=False, threaded=True) # Threaded is crucial
