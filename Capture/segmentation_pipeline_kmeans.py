...
*** MODIFIED: Split into create_hsv_nir_mask and segment_leaves_from_mask
    to allow VI heatmaps to be generated without running expensive watershed. ***
"""
...
# --- *** NEW FAST MASKING FUNCTION *** ---
def create_hsv_nir_mask(
    rgb_aligned_bgr: np.ndarray,
    nir_aligned: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Creates a simple, fast binary leaf mask using HSV + NIR thresholding.
    This is used for generating VI heatmaps without running expensive segmentation.
    
    Returns: Tuple (cleaned_mask_uint8, hsv_nir_debug_img)
    """
    ...
    print(f"  [Segment Simple] NIR Threshold: {nir_threshold}")
    ...
    print(f"  [Segment Simple] HSV+NIR mask generated in {time.time() - start_time:.3f}s")
    
    return cleaned_mask, hsv_nir_debug_img
# --- *** END OF FAST MASKING FUNCTION *** ---


# --- *** MODIFIED SEGMENTATION FUNCTION (NOW REQUIRES MASK) *** ---
def segment_leaves_from_mask(
    cleaned_mask: np.ndarray,
    rgb_aligned_bgr: np.ndarray, # Still needed for debug image size
    ...
    **kwargs # Accepts extra args like n_clusters but ignores them
    ...
    """
    Segments leaves from a pre-calculated binary mask using Watershed.
    This is the computationally expensive step and should only be run for AI inference.
    
    Returns: Tuple ([(leaf_id, mask)], watershed_debug_img)
    """
    start_time = time.time()
    ...
    if coords.size == 0:
        ...
        if np.sum(cleaned_mask) < min_leaf_area: 
            ...
            return [], np.zeros_like(rgb_aligned_bgr)
    ...
    print(f"  [Segment] Watershed separation finished in {total_segment_time:.3f}s. Found {len(all_leaf_masks_watershed)} valid leaves.")
    
    # Return: 
    # 1. List of (id, mask) tuples
    # 2. The final color-coded watershed image
    return all_leaf_masks_watershed, watershed_debug_img
...
def main():
    ...
    for pair_info in all_image_pairs_with_labels:
        ...
        try:
            ...
            # Step 1: Create the simple mask
            cleaned_mask, hsv_nir_debug = create_hsv_nir_mask(
                rgb_aligned_bgr, nir_aligned
            )
            # Step 2: Run watershed segmentation on the simple mask
            robust_masks, watershed_debug = segment_leaves_from_mask(
                cleaned_mask,
                rgb_aligned_bgr
            )
